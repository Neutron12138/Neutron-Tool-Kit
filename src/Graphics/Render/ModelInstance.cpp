...
namespace ntk
{
    namespace Graphics
    {
        ModelInstance::ModelInstance(const std::shared_ptr<VertexArrayObject> &VAO) : Sprite(), m_VAO(VAO) {}
        ModelInstance::ModelInstance(const std::shared_ptr<VertexArrayObject> &VAO, const std::shared_ptr<DrawInfo> &draw_info) : Sprite(), m_VAO(VAO), m_draw_info(draw_info) {}
