...
namespace ntk
{
    namespace Graphics
    {
        ...
        template <typename m_VertexType, typename m_NormalType, typename m_TexCoordType>
        VertexArrayObject Model<m_VertexType, m_NormalType, m_TexCoordType>::generate_VAO(const Program &program, const VertexAttribDescriber &describer)
        {
            VertexArrayObject VAO;
            VAO.generate();
            VAO.bind();
            VertexBufferObject VBO1 = VertexBufferObject::createVBO_single(m_vertices, glGetAttribLocation(program.get_objectID(), describer.get_name(VertexAttributeEnum::VertexAttributeLocation::Position).data()), GL_STATIC_DRAW);
            VertexBufferObject VBO2 = VertexBufferObject::createVBO_single(m_normals, glGetAttribLocation(program.get_objectID(), describer.get_name(VertexAttributeEnum::VertexAttributeLocation::Normal).data()), GL_STATIC_DRAW);
            VertexBufferObject VBO3 = VertexBufferObject::createVBO_single(m_texCoords, glGetAttribLocation(program.get_objectID(), describer.get_name(VertexAttributeEnum::VertexAttributeLocation::TexCoord).data()), GL_STATIC_DRAW);
            VAO.unbind();
            VBO1.unbind();
            VBO1.delete_object();
            VBO2.delete_object();
