...
namespace ntk
{
    namespace Graphics
    {
        ...
        bool TrueTypeFont<m_GlyphType, m_GlyphCollectionType>::load_font(const std::string &filename)
        {
            ...
            {
                Instance::log.loge("TrueTypeFont<m_GlyphType,m_GlyphCollectionType>", Utils::to_string("Unable to load Face from file:\"", filename, "\""));
        ...
        bool TrueTypeFont<m_GlyphType, m_GlyphCollectionType>::load_glyph(std::uint32_t code, std::uint32_t size, GlyphType &result)
        {
            ...
            {
                Instance::log.loge("TrueTypeFont<m_GlyphType,m_GlyphCollectionType>", Utils::to_string("Unable to load glyph:", code));
                return false;
            }
