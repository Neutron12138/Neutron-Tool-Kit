...
namespace ntk
{
    namespace Graphics
    {
        ...
        public:
            ...
            /// @brief 绘制矩形
            /// @param position 矩形起始点（左上角）
            /// @param size 矩形尺寸
            virtual void draw_rect(const glm::vec2 &position, const glm::vec2 &size) = 0;

            /// @brief 绘制圆
            /// @param position 圆的位置（圆心）
            /// @param radius 圆的半径
            virtual void draw_circle(const glm::vec2 &position, float radius) = 0;
