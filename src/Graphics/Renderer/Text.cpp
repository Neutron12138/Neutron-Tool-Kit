        ...
        void Text<m_StringType, m_FontType>::render(const glm::vec2 &scale)
        {
            ...
            for (std::size_t i = 0; i < m_text.length(); i++)
            {
                ...
                if (gc == nullptr)
                {
                    ...
                    {
                        Instance::log.loge("Text", Utils::to_string("Unable to load glyph:", ch, ",size:", m_text_size));
                        return;
                    }
                }
                else
        ...
        int measure_text_width(const StringType &text, std::uint32_t size, FontType *font)
        {
            ...
            for (std::size_t i = 0; i < text.length(); i++)
            {
                ...
                if (gc == nullptr)
                {
                    ...
                    {
                        Instance::log.loge("measure text width", Utils::to_string("Unable to load glyph:", ch, ",size:", size));
                        return -1;
                    }
                }
                else
