...
namespace ntk
{
    namespace Graphics
    {
        ...
        template <typename m_StringType, typename m_FontType>
        void Text<m_StringType, m_FontType>::render(float scale)
        {
            render(glm::vec2(scale, scale));
        }

        template <typename m_StringType, typename m_FontType>
        void Text<m_StringType, m_FontType>::render(glm::vec2 scale)
        {
            // 判断是否每个字形都可用
            ...
            for (std::size_t i = 0; i < m_text.length(); i++)
            {
                ...
                glyph->bind_texture();
                m_VAO.bind();
                m_VBOs.at(VBOIndex::Vertices).bind();
                m_VBOs.at(VBOIndex::Vertices).sub_data(vertices);
