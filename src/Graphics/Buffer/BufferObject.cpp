...
namespace ntk
{
    namespace Graphics
    {
        ...
        void BufferObject::sub_data(GLintptr offset, GLsizeiptr size, const void *data)
        {
            glBufferSubData(m_buffer_type, offset, size, data);
        }

        void BufferObject::sub_data(GLsizeiptr size, const void *data)
        {
            sub_data(0, size, data);
        }

        template <typename DataType>
        void BufferObject::sub_data(GLintptr offset, const std::vector<DataType> &data)
        {
            sub_data(offset * sizeof(DataType), data.size() * sizeof(DataType), data.data());
        }

        template <typename DataType>
        void BufferObject::sub_data(const std::vector<DataType> &data)
        {
            sub_data(0, data);
        }
